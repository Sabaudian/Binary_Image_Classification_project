...
def plot_view_dataset(train_ds, show_on_screen=True, store_in_folder=True):
    ...
    plt.suptitle(t="Dataset Visual Example with Class Labels", fontsize=18, weight="bold")
...
def plot_visual_prediction(model, model_name, x_test, y_test, show_on_screen=True, store_in_folder=True):
    ...
    random.seed(300)  # Set random seed
    ...
    # Compute probability
    prediction_probability = predicts * 100
    ...
    for i in range(min(subplot_rows * subplot_cols, len(x_test))):
        ...
        # Add Visual aid to check the correctness of the prediction, for an intuitive read of the data
        ...
        if predicted_class_labels[i] == "chihuahua":
        ...
        plt.title(
            label="TRUE: {}\nPREDICTED: {} ({:.4g}%)".format(true_class_label, predicted_class_labels[i],
                                                             probability_display),
